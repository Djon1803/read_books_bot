...
def get_inline_read_book(
    user: User, book: Book, index: int, lexicon: dict
) -> InlineKeyboardMarkup:
    # Создаем объекты инлайн-кнопок
    btn_back = InlineKeyboardButton(
        text=lexicon["page_back"], callback_data="page_back"
    )
    ...
    btn_next = InlineKeyboardButton(
        text=lexicon["page_next"], callback_data="page_next"
    )
